...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = m32rFileSystem.MyFileSystemFactory.class)
public class m32rFileSystem implements GFileSystem {
	 ...
	 */
	public m32rFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + m32rFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<m32rFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public m32rFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			m32rFileSystem fs = new m32rFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
